...
modelfit <- function(GDINA.obj,CI=0.90,ItemOnly=FALSE)
{
  ...
  p <- c(colMeans(dat,na.rm = TRUE), crossp[lower.tri(crossp)]) # length of nitems + nitems*(nitems-1)/2
  ...
  difr <- cor(dat,use="pairwise.complete.obs")-(Xi$bi-Xi$uni%*%t(Xi$uni))/(se%*%t(se))
  ...
  if(ItemOnly & nitems*(nitems+1)/2-extract(GDINA.obj,"npar.item")<0){
      warning("M2 statistic cannot be calculated - Degrees of freedom are too low.",call. = FALSE)
    }else if(!ItemOnly & nitems*(nitems+1)/2-extract(GDINA.obj,"npar")<0){
    warning("M2 statistic cannot be calculated - Degrees of freedom are too low.",call. = FALSE)
  }else{
    # parameter locations
    patt <- eta(as.matrix(Q))
    Mj <- list()
    for(s in 1:ncat) Mj[[s]] <- designmatrix(Kj[s],models[s])

    parloc <- matrix(c(cumsum(c(1,unlist(lapply(Mj,ncol))[-ncat])),
                       cumsum(unlist(lapply(Mj,ncol)))),ncol = 2)
    # partial s/ partial d
    ...
    for (j in 1:nitems) {
      ...
      if(length(locj)>1){ # sequential model
        ...
        seq_component[locj,] <- (rep(1,nrow(pfs))%o%expected.score[j,]-cumpf)/pfs
    ...
    for(j in 1:nitems){
      locj <- which(item.no==j)
      for(sj in locj){
        delta11[j,parloc[sj,1]:parloc[sj,2]] <- colSums(extMj[[sj]]*post*seq_component[sj,])
      }
      ...
      for (i in 1:nitems){
        if(j<i){
          ...
          for(sj in locj){
            delta21[loc,parloc[sj,1]:parloc[sj,2]] <- colSums(extMj[[sj]]*post*seq_component[sj,]*expected.score[i,])
          }
          for(si in loci){
            delta21[loc,parloc[si,1]:parloc[si,2]] <- colSums(extMj[[si]]*post*seq_component[si,]*expected.score[j,])
          }
    ...
    # cat("Dim of delt is",nrow(delt),"and",ncol(delt),"\n")
